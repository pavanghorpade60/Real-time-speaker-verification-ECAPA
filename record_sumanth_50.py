...
OUTPUT_FOLDER = "recordings/sumanth"
os.makedirs(OUTPUT_FOLDER, exist_ok=True)

print(f"\nRecording {NUM_SAMPLES} clean samples for SUMANTH...\n")
...
i = 1
while i <= NUM_SAMPLES:
    input(f"\nPress ENTER to record SUMANTH sample {i}/{NUM_SAMPLES}")
    ...
    file_path = os.path.join(OUTPUT_FOLDER, f"sumanth_{i:02d}.wav")
    sf.write(file_path, trimmed, SAMPLE_RATE)

    print(f"Saved: sumanth_{i:02d}.wav | Duration: {trimmed_duration:.2f}s")
    i += 1

print(f"\nAll {NUM_SAMPLES} SUMANTH samples recorded successfully!")